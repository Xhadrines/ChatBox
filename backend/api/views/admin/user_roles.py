...
class AdminUserRolesViews(APIView):
    ...
    def get(self, request, pk=None):
        if pk:
            ...
            serializer = AdminUserRolesSerializer(user_role)
            return Response(serializer.data)

        user_roles = self.accessor.get_all()
        serializer = AdminUserRolesSerializer(user_roles, many=True)
        return Response(serializer.data)

    def post(self, request):
        serializer = AdminUserRolesSerializer(data=request.data)
        if serializer.is_valid():
            ...
            return Response(
                AdminUserRolesSerializer(user_role).data, status=status.HTTP_201_CREATED
            )
    ...
    def put(self, request, pk=None):
        ...
        serializer = AdminUserRolesSerializer(user_role, data=request.data)
        if serializer.is_valid():
            data = serializer.validated_data
            updated_role = self.accessor.update(pk, **data)
            return Response(AdminUserRolesSerializer(updated_role).data)
    ...
    def patch(self, request, pk=None):
        ...
        serializer = AdminUserRolesSerializer(
