...
class GenericCRUDView(APIView):
    ...
    def get_object(self, pk):
        if self.lookup_param_is_user:
            user_accessor = self.user_accessor_class()
            user = user_accessor.get_by_id(pk)
            if not user:
                return None
            return self.get_lookup_fn()(user)
        else:
            return self.get_lookup_fn()(pk)

    def delete_object(self, pk):
        if self.lookup_param_is_user:
            user = self.user_accessor_class().get_by_id(pk)
            if not user:
                return 0
            return self.get_delete_fn()(user)
        else:
            return self.get_delete_fn()(pk)
